...
const char * const BITCOIN_CONF_FILENAME = "bastoji.conf";
const char * const BITCOIN_PID_FILENAME = "bastojid.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "bastoji" is a composite category enabling all Bastoji-related debug output
                if(ptrCategory->count(std::string("bastoji"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BastojiCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BastojiCore
    // Mac: ~/Library/Application Support/BastojiCore
    // Unix: ~/.bastojicore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BastojiCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BastojiCore";
#else
    // Unix
    return pathRet / ".bastojicore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty bastoji.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override bastoji.conf
