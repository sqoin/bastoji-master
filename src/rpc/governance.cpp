...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in bastoji.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("bastoji.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Bastoji features */
    { "bastoji",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "bastoji",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "bastoji",               "gobject",                &gobject,                true,  {} },
    { "bastoji",               "voteraw",                &voteraw,                true,  {} },
